...
// The standard manager's work board -- a deliberate, standalone copy of
// WorkerDashboard.jsx rather than a shared component, so a manager's board can
// change without touching what workers see.
//
// One of the layouts in workLayouts.js. ManagerWorkDashboard picks which one
// to render; this file only knows how to draw itself.
...
export default function StandardWorkBoard() {
  const { getToken } = useAuth();
  ...
  const filteredPersonal = tickets
    ...
    .filter((t) => {
      ...
      // "all" -- a recurring ticket's own monthly instance is represented by
      // its recurring ticket card instead, so it's excluded here to avoid
      // showing the same series twice.
